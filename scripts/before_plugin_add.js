...
module.exports = function (context) {
    ...
    return new Promise((resolve, reject) => {
        ...
        //console.log("rootPath", rootPath, context.opts.plugin.dir);
        var configXmlPath = path.join(rootPath, 'config.xml');
        var configXml = fs.readFileSync(configXmlPath, "utf-8");
        var pluginXml = fs.readFileSync(context.opts.plugin.dir + "/plugin.xml", "utf-8");
                ...
                parseString(pluginXml, function (err, pluginXmlParsed) {
                    if (err) {
                        ...
                        if (p) {
                            ...
                            fs.writeFile(context.opts.plugin.dir + "/plugin.xml", xml, function (err, data) {
                                if (err) {
                                    console.log(err);
                                    reject(err)
                                } else {
